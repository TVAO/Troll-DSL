...
let updateGlobal x v = globalEnv <- update globalEnv x v
...
let rec eval s localEnv =
    ...
    | Symbol x -> // predefined or variable
        if List.contains x operators then
            s // operators evaluate to themselves
        else
    ...
    | Cons (Symbol "if", es) ->
        ...
        | Cons (e1, Cons (e2, ess)) ->
            if eval e1 localEnv <> Nil then
                eval e2 localEnv
            else
                eval (Cons(Symbol "if", ess)) localEnv
    ...
    | Cons (Symbol "define", Cons (Symbol x, Cons (e, Nil))) ->
        ...
        if List.contains x predefined then
            ...
            (updateGlobal x (eval e localEnv)
             Symbol x)
    ...
    | Cons (Symbol "load", Cons (Symbol f, Nil)) ->
        ...
        let infile =
            try
                new System.IO.StreamReader(f + ".le")
            with _ -> raise (Lerror("Could not open file " + f + ".le"))
...
and applyFun (fnc, pars, localEnv) =
    ...
    | Symbol x when (List.contains x binops) ->
        ...
        match pars with
        | Cons (v1, Cons (v2, Nil)) -> applyBinop x (v1, v2, localEnv)
        | _ -> raise (Lerror("Wrong number of arguments to " + x))
        ...
        with Lerror message ->
            raise (
                Lerror(
                    message
                    + "\n! when applying "
                    + showSexpIndent (Cons(Symbol "lambda", rules)) 16 16
                    + "\n! to "
...
and applyVarop x vs =
    ...
    | ("+", Nil) -> Num 0
    | ("+", Cons (Num n, vs1)) ->
        match applyVarop x vs1 with
        | Num m -> Num(n + m)
        | _ -> Nil
    | ("-", Nil) -> Num 0
    | ("-", Cons (Num n, Nil)) -> Num(-n)
    | ("-", Cons (Num n, vs1)) ->
        match applyVarop "+" vs1 with
        | Num m -> Num(n - m)
        | _ -> Nil
    | ("*", Nil) -> Num 1
    | ("*", Cons (Num n, vs1)) ->
...
and matchPattern p v =
    ...
    | (Symbol x, w) ->
        if List.contains x specials then
            raise (Lerror(x + " can not be used as pattern"))
        else if List.contains x operators then
            (if Symbol x = w then Some [] else None)
        else
            Some [ (x, w) ]
    | (Cons (Symbol "quote", Cons (v, Nil)), w) -> if v = w then Some [] else None
